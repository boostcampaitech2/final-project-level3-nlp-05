...
def retrieve_main_title(df, centers, dict):
    ...
    feature_title = []
    feature_article = []
    for i in range(1,len(centers)-3):
        min_idx,min =  0,1
...
def get_cluster_details_dbscan(centers,feature_names, feature_title, feature_article, top_n_features=10):
    ...
    for cluster_num in range(1,len(centers)-3): # -1, 0 제외
...
def retrieve_json(cluster_details, retrive_topk_clusters):
    """ 선정된 클러스터 별 기사들 json 파일로 변환 """
#     dict = {}
#     for cluster_num in retrive_topk_clusters:
#         title = cluster_details[cluster_num]['title']
#         article = cluster_details[cluster_num]['article']
    # index 있으면 좋음, 카테고리 정보 추가(int나 str)
    return # json 파일로 저장
...
if __name__ == "__main__":
    ...
    daily_category_specific_dir = f'./articles/daum_articles_{args.date}_{category_list[args.category]}.json'
